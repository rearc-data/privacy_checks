...
class PyCanonChecker(BasicChecker):
    ...
    def check_k_anonymity(self, df: DataFrame):
        ...
        if k < self.k_threshold:
            raise Exception(
                'K-anonymity check has failed.  Either lower your k threshold or employ techniques to further anonymize this dataset if possible.')
        else:
            return (True, k)
    ...
    def check_alpha_k_anonymity(self, df: DataFrame):
        ...
        if not alpha_k:
            ...
            raise Exception(
                f'(alpha, k)-anonymity check has failed.  Either lower your k threshold or employ techniques to further anonymize this dataset if possible. (alpha, k)-anonymity value: {alpha_k}')
        else:
            return (True, alpha_k)
    ...
    def check_l_diversity(self, df: DataFrame):
        ...
        if not l_div:
            ...
            raise Exception(
                f'l-diversity check has failed.  Either lower your l threshold or employ techniques to further anonymize this dataset if possible. l-diversity value: {l_div}')
        else:
            return (True, l_div)

    def check_entropy_l_diversity(self, df: DataFrame):
        entropy_l_div = anonymity.entropy_l_diversity(
            df, self.qi, self.sa)
        if not entropy_l_div:
            raise Exception('entropy l-diversity check has failed.')
        else if self.entropy_l_threshold < entropy_l_div:
            raise Exception(
                f'entropy l-diversity check has failed.  Either lower your l threshold or employ techniques to further anonymize this dataset if possible. entropy l-diversity value: {entropy_l_div}')
        else:
    ...
    def check_c_l_diversity(self, df: DataFrame):
        ...
        if not c_l_div:
            ...
            raise Exception(
                f'(c, l)-diversity check has failed.  Either lower your l threshold or employ techniques to further anonymize this dataset if possible. (c, l)-diversity value: {c_l_div}')
        else:
            return (True, c_l_div)

    def check_basic_beta_likeness(self, df: DataFrame):
        beta_likeness = anonymity.basic_beta_likeness(
            df, self.qi, self.sa)
        if not beta_likeness:
            raise Exception('basic beta-likeness check has failed.')
        else if self.beta_threshold < beta_likeness:
            raise Exception(
                f'basic beta-likeness check has failed.  Either lower your beta threshold or employ techniques to further anonymize this dataset if possible. basic beta-likeness value: {beta_likeness}')
        else:
            return (True, beta_likeness)

    def check_enhanced_beta_likeness(self, df: DataFrame):
        enhanced_beta_likeness = anonymity.enhanced_beta_likeness(
            df, self.qi, self.sa)
        if not enhanced_beta_likeness:
            ...
            raise Exception(
                f'enhanced beta-likeness check has failed.  Either lower your beta threshold or employ techniques to further anonymize this dataset if possible. enhanced beta-likeness value: {enhanced_beta_likeness}')
        else:
    ...
    def check_t_closeness(self, df: DataFrame):
        ...
        if not t_closeness:
            ...
            raise Exception(
                f't-closeness check has failed.  Either lower your t threshold or employ techniques to further anonymize this dataset if possible. t-closeness value: {t_closeness}')
        else:
            return (True, t_closeness)

    def check_delta_disclosure_privacy(self, df: DataFrame):
        delta_disclosure_privacy = anonymity.delta_disclosure(
            df, self.qi, self.sa)
        if not delta_disclosure_privacy:
            ...
            raise Exception(
                f'delta-disclosure privacy check has failed.  Either lower your delta threshold or employ techniques to further anonymize this dataset if possible. delta-disclosure privacy value: {delta_disclosure_privacy}')
        else:
